...
use dynamo_http_server::lifecycle::{
    InflightPermit, ServiceObserver, ServiceStage, echo_request_id_header, track_inflight_response,
};
use dynamo_http_server::metrics::{Metrics, Registry, register_worker_timing_metrics};
use dynamo_http_server::server::{HttpServer, HttpServerConfig};
...
async fn track_inflight_inference(
    ...
    track_inflight_response(next.run(request).await, permit)
}
